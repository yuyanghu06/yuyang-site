...
- The FaceTime shell's browser-only avatar renderer lives under `src/components/avatar-view/`. It loads `public/models/yuyang-avatar-idle-loop-v2.glb` and attaches the approved closed-mouth smile atlas tile to the `Head` bone. Every animation that should keep the avatar planted must use `createFixedBodyAnimationClip` from `fixed-body-animation.ts`; this shared policy removes root, torso, hip, and leg motion while retaining the neck/head and complete arm, hand, and finger chains. Preserve the Blender-Z-up to glTF-Y-up coordinate conversion when changing its camera or face canvas.
