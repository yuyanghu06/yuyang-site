...
- `blog.css` gives the crawlable blog index and article pages a dimensional editorial-paper background with broad amber, sage, and center-light gradients plus a generated, repeating crumpled-paper texture that scrolls with the document. It restores document scrolling for those routes, keeps the index back link close to the article grid, and collapses that grid for narrow screens.
