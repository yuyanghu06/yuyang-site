...
- The former oversized placeholder arch has been replaced by a low-detail open-data model derived from OpenStreetMap way `248166269`: its actual footprint and rotation, mapped 20.5-meter height, and correct north-gate location are used directly.
...
3. Decide whether the low-detail OSM-derived arch is sufficient for this stage or should be replaced by the CC BY Washington Square Park model discovered on Sketchfab.
